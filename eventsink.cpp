...
CEventSink::CEventSink(CRScriptCore* pEngine)
...
/*
		IDispatchEx* pEx;
		if (m_pParentDisp->QueryInterface(IID_IDispatchEx, (void**)&pEx) == S_OK)
		{
			hr = pEx->GetDispID(bstrName, grfdex, pid);
			pEx->Release();
		}
*/
	}
            ...
            /* [unique][in] */ IServiceProvider __RPC_FAR *pspCaller)
{
	...
	if (hr == DISP_E_MEMBERNOTFOUND)
	{
		...
		if (m_pParentDisp->QueryInterface(IID_IDispatchEx, (void**)&pEx) == S_OK)
		{
			hr = pEx->InvokeEx(id, lcid, wFlags, pdp, pvarRes, pei, pspCaller);
			pEx->Release();
		}
#endif
	}
